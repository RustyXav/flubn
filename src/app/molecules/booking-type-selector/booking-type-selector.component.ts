...
export class BookingTypeSelectorComponent {
  ...
  GetBookingOptions() {
    const retVal = [];
    const filtered0 = this.router.config.filter(route => route.path === homeRoutePath);
    if (filtered0 && filtered0.length === 1) {
      const homeRoute = filtered0[0];
      if (homeRoute && homeRoute.children) {
        const filtered1 = homeRoute.children.filter(route => route.path === bookingRoutePath);
        if (filtered1 && filtered1.length === 1) {
          const bookingRoute = filtered1[0];
          if (bookingRoute && bookingRoute.children) {
            retVal.push(...bookingRoute.children.map(route => ({
              label: route.data?.['label'],
              relativePath: encodeURIComponent(homeRoutePath) + '/' + encodeURIComponent(bookingRoutePath) + '/' + encodeURIComponent(route.path ?? ''),
              checked: false
            })));
          }
        }
  ...
  SetBookingOptionsChecked(currentURL: string) {
    const filteredBookingOptions = this.bookingOptions.filter(bookingOption => bookingOption.relativePath === currentURL);
    ...
    if (filteredBookingOptions.length) {
      this.bookingOptions.filter(bookingOption => bookingOption != filteredBookingOptions[0]).forEach(b => b.checked = false);
